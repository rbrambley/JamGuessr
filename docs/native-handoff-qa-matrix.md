...
| Host reconnect recovery | Refresh/rejoin host during native round | Phase recovers safely; clients remain stable | [x] | Provisional pass (2026-05-14): pre-patch run failed with reconnect-fallback write conflicts; post-patch synthetic contention stress passed (48/48 writes, 0 retryable errors). Local patched reload check (room `S827`) recovered `launching` + `host_missing` to native `ready` without reconnect-fallback `failed-precondition`. Hosted fresh-runtime retest also recovered cleanly with no old reconnect-fallback conflict reproduced. Final sign-off still requires full manual multi-device verification. |
